...
import {
  ...
  CircleDollarSign, ClipboardCheck, Copy, Database, FolderKanban, LayoutDashboard,
  Mail, Menu, Plus, Search, Settings, ShieldCheck, Trash2, UserPlus, Users, X
} from 'lucide-react'
import { api } from './api'
...
// Invoices are tracked inside the project they belong to, so they have no page of their own.
...
const withCommitted = (projects:Project[],lines:BudgetLine[],projectId:string)=>{
  const committed=lines.filter(line=>line.projectId===projectId).reduce((total,line)=>total+Number(line.committed||0),0)
  return projects.map(p=>p.id===projectId?{...p,committed,updated:new Date().toISOString()}:p)
}
...
function App(){
  ...
  const [modal,setModal] = useState<'project'|'item'|null>(null)
  ...
  const saveBudgetLine=(line:BudgetLine)=>setData(d=>{
    const budgetLines=d.budgetLines.some(item=>item.id===line.id)?d.budgetLines.map(item=>item.id===line.id?line:item):[...d.budgetLines,line]
    return {...d,budgetLines,projects:withCommitted(d.projects,budgetLines,line.projectId)}
  })
  const deleteBudgetLine=(projectId:string,id:string)=>setData(d=>{
    const budgetLines=d.budgetLines.filter(item=>item.id!==id)
    return {...d,budgetLines,projects:withCommitted(d.projects,budgetLines,projectId)}
  })
  const saveCategoryFunding=(funding:BudgetCategoryFunding)=>setData(d=>{
  ...
  return <div className="app-shell">
    ...
    {modal&&<Modal title={modal==='project'?'Add project':'Add outstanding item'} close={()=>setModal(null)}><CreateForm kind={modal} projects={data.projects} teams={teams.teams} selected={selected} defaultOwner={auth?.name||''} save={(payload)=>{setData(d=>modal==='project'?{...d,projects:[payload as Project,...d.projects]}:{...d,items:[payload as Item,...d.items]});setModal(null)}}/></Modal>}
  </div>
}
...
function Dashboard({data,openProject,setView}:{data:Store;openProject:(id:string)=>void;setView:(v:View)=>void}){
  ...
  // Vendor closeout letters still owed across every project that has not closed.
  const outstandingLetters=open.flatMap(p=>vendorReadiness(summarizeVendors(data.budgetLines.filter(l=>l.projectId===p.id),data.invoices.filter(i=>i.projectId===p.id),data.vendors.filter(v=>v.projectId===p.id))).outstandingLetters)
...
function ProjectDetail({project,data,teamName,readOnly,updateProject,updateItem,addActivity,back,addItem,saveBudgetLine,deleteBudgetLine,saveCategoryFunding,saveVendor,saveInvoice,deleteInvoice}:{project:Project;data:Store;teamName:string;readOnly:boolean;updateProject:(id:string,p:Partial<Project>)=>void;updateItem:(id:string,p:Partial<Item>)=>void;addActivity:(pid:string,a:string,d:string)=>void;back:()=>void;addItem:()=>void;saveBudgetLine:(line:BudgetLine)=>void;deleteBudgetLine:(projectId:string,id:string)=>void;saveCategoryFunding:(funding:BudgetCategoryFunding)=>void;saveVendor:(record:VendorRecord)=>void;saveInvoice:(invoice:Invoice)=>void;deleteInvoice:(id:string)=>void}){
  ...
  const changeStatus=(next:ProjectStatus)=>{
    const pending=readiness.outstandingLetters
    if((next==='Ready to Close'||next==='Closed')&&pending.length>0&&!confirm(`${pending.length} closeout letter${pending.length===1?'':'s'} still outstanding (${pending.map(v=>v.name).join(', ')}). Set the project to ${next} anyway?`))return
    updateProject(project.id,{status:next})
  }
  return <><button className="back" onClick={back}><ArrowLeft/>Back to projects</button><div className="project-hero"><div><span className="eyebrow">Project {project.number}</span><h1>{project.name}</h1><p>{project.location} · {project.department}</p></div><div className="hero-actions">{readOnly?<span className={cls(project.status)}>{project.status}</span>:<><select value={project.status} onChange={e=>changeStatus(e.target.value as ProjectStatus)}>{projectStatuses.map(s=><option key={s}>{s}</option>)}</select><button className="secondary" onClick={()=>{addActivity(project.id,'Follow-up logged',project.nextAction);updateProject(project.id,{updated:new Date().toISOString()})}}>Log follow-up</button></>}</div></div>
  <div className="progress-panel"><div><strong>{progress}% complete</strong><span>{resolved} of {items.length} requirements resolved · {readiness.settledLetters} of {readiness.vendors} vendor letter{readiness.vendors===1?'':'s'} settled</span></div><div className="progress"><i style={{width:`${progress}%`}}/></div></div>
...
function CreateForm({kind,projects,teams,selected,defaultOwner,save}:{kind:'project'|'item';projects:Project[];teams:Team[];selected:string|null;defaultOwner:string;save:(v:Project|Item)=>void}){const [form,setForm]=useState<Record<string,string>>({projectId:selected||projects[0]?.id||'',due:date(7),status:kind==='project'?'Needs Attention':'Not Started'});const set=(k:string,v:string)=>setForm(f=>({...f,[k]:v}));const submit=(e:React.FormEvent)=>{e.preventDefault();if(kind==='project')save({id:uid(),name:form.name||'Untitled project',number:form.number||'TBD',teamId:form.teamId||'',department:form.department||'',location:form.location||'',manager:form.manager||defaultOwner,vendor:form.vendor||'',budget:Number(form.budget||0),committed:Number(form.committed||0),target:form.due,status:form.status as ProjectStatus,nextAction:form.nextAction||'Review project closeout requirements',owner:form.owner||defaultOwner,due:form.due,updated:new Date().toISOString()});else save({id:uid(),projectId:form.projectId,title:form.title||'Untitled item',type:form.type||'Other',responsible:form.responsible||'',owner:form.owner||defaultOwner,amount:Number(form.amount||0),due:form.due,priority:(form.priority||'Medium') as Item['priority'],status:form.status as ItemStatus,notes:form.notes||'',followUp:''})};return <form className="form" onSubmit={submit}>{kind!=='project'&&<label>Project<select value={form.projectId} onChange={e=>set('projectId',e.target.value)}>{projects.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select></label>}{kind==='project'?<><label>Project name<input required onChange={e=>set('name',e.target.value)}/></label><div className="form-row"><label>Project number<input onChange={e=>set('number',e.target.value)}/></label><label>Department<input onChange={e=>set('department',e.target.value)}/></label></div><div className="form-row"><label>Team<select value={form.teamId||''} onChange={e=>set('teamId',e.target.value)}><option value="">Unassigned</option>{teams.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label><label>Location<input onChange={e=>set('location',e.target.value)}/></label></div><div className="form-row"><label>Project manager<input defaultValue={defaultOwner} onChange={e=>set('manager',e.target.value)}/></label><label>Vendor<input onChange={e=>set('vendor',e.target.value)}/></label></div><div className="form-row"><label>Budget<input type="number" onChange={e=>set('budget',e.target.value)}/></label><label>Committed<input type="number" onChange={e=>set('committed',e.target.value)}/></label></div><label>Next action<input onChange={e=>set('nextAction',e.target.value)}/></label></>:<><label>Item title<input required onChange={e=>set('title',e.target.value)}/></label><div className="form-row"><label>Type<select onChange={e=>set('type',e.target.value)}><option>Final Invoice</option><option>Retainage</option><option>Change Order</option><option>Purchase Order</option><option>Credit / Refund</option><option>Lien Waiver</option><option>Permit Closure</option><option>Warranty Document</option><option>Final Approval</option><option>Other</option></select></label><label>Priority<select onChange={e=>set('priority',e.target.value)}><option>Medium</option><option>High</option><option>Low</option></select></label></div><label>Responsible party<input onChange={e=>set('responsible',e.target.value)}/></label><label>Amount<input type="number" onChange={e=>set('amount',e.target.value)}/></label><label>Notes<textarea rows={3} onChange={e=>set('notes',e.target.value)}/></label></>}<div className="form-row"><label>Due date<input type="date" value={form.due} onChange={e=>set('due',e.target.value)}/></label><label>Status<select value={form.status} onChange={e=>set('status',e.target.value)}>{(kind==='project'?projectStatuses:itemStatuses).map(s=><option key={s}>{s}</option>)}</select></label></div><button className="primary full" type="submit">Create {kind}</button></form>}
